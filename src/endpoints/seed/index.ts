...
// Lỗi revalidation của Next.js là bình thường khi seed database mà không chạy server
// tức là chạy `yarn seed` cục bộ thay vì dùng admin UI trong một app đang hoạt động
// App không chạy để revalidate các page nên các API route không khả dụng
// Các thông báo lỗi này có thể bỏ qua: `Error hitting revalidate route for...`
export const seed = async ({
  ...
  // cần xóa thư mục media trước khi seed
  // cũng như các collection và global
  // lý do là vì `yarn seed` sẽ drop database
  // nhưng endpoint tùy chỉnh `/api/seed` thì không
  ...
  // Cart này đang open vì vừa được tạo
  ...
  // Cart bị abandoned vì được tạo từ rất lâu trong quá khứ
  ...
  // Cart đã được purchased vì có ngày purchasedAt
